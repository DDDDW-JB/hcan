...
class CARN(nn.Module):
    def __init__(self, d=64):
        ...
        multi_scale = True
        group = 1
        # self.squeezeExcitation1=SqueezeExcitation(d * 2)
        # self.squeezeExcitation2=SqueezeExcitation(d * 3)
        # self.squeezeExcitation3=SqueezeExcitation(d * 4)
    ...
    def forward(self, x, scale):
        x = self.entry(x) #[64, 64, 30, 30]

        c0 = o0 = x

        b1 = self.b1(o0) #[64, 64, 30, 30]
        c1 = torch.cat([c0, b1], dim=1) #[64, 128, 30, 30]
        # c1=self.squeezeExcitation1(c1)
        o1 = self.c1(c1)  #[64, 64, 30, 30]

        b2 = self.b2(o1)  #[64, 64, 30, 30]
        c2 = torch.cat([c1, b2], dim=1)  #[64, 192, 30, 30]
        # c2=self.squeezeExcitation2(c2)
        o2 = self.c2(c2)   #[64, 64, 30, 30]

        b3 = self.b3(o2)  #[64, 64, 30, 30]
        c3 = torch.cat([c2, b3], dim=1)   #[64, 256, 30, 30]
        # c3=self.squeezeExcitation3(c3)
        o3 = self.c3(c3)  #[64, 64, 30, 30]

        out = self.upsample(o3, scale=scale) #[64, 64, 60, 60]
        out = self.exit(out)   #[64, 1, 60, 60]
        return out


class Bicubic_plus_plus(nn.Module):
    def __init__(self, sr_rate=2,channel=32):
        super(Bicubic_plus_plus, self).__init__()
        self.conv0 = nn.Conv2d(1, channel, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        self.conv_out = nn.Conv2d(channel, (2 * sr_rate) ** 2 * 1, kernel_size=3, padding=1, bias=False)
        self.Depth2Space = nn.PixelShuffle(2 * sr_rate)
        self.act = nn.LeakyReLU(inplace=True, negative_slope=0.1)

    def forward(self, x):
        x0 = self.conv0(x)  #[64, 32, 20, 20]
        x0 = self.act(x0)
        x1 = self.conv1(x0)
        x1 = self.act(x1)
        x2 = self.conv2(x1)
